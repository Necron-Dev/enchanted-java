  ...
  public void apply(Project project) {
    project.getPlugins().withId("java", plugin -> {
      ...
      javaExtension.getSourceSets().all(sourceSet -> {
        ...
        var compileTaskProvider = project.getTasks().named(compileTaskName, JavaCompile.class, compileTask -> {
          compileTask.getDestinationDirectory().set(intermediateOutputDir);

        });

        var enchantTaskProvider = project.getTasks().register(enchantTaskName, EnchantTask.class, enchantTask -> {
